...
def get_assigned_personel_for_patient(db, pesel):
    cur = db.execute('select e.fname, e.lname, p.name, e.rights from employees e join positions p on p.id = e.position_id join assignments a on a.employee_id = e.id join files f on f.id = a.fil_id join patients pat on f.patient_pesel = pat.pesel where pat.pesel = ? and f.discharge_d is null', [pesel])
    return cur.fetchall()
